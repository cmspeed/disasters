    ...
    try:
        ...
        if log:
            ...
            diff.attrs["DESCRIPTION"] = (
                "Log Ratio Difference (Later / Earlier) for RTC-S1"
            )

            # Handle Nodata
            input_nodata_mask = xr.where(
                da_later.isnull() | da_early.isnull(), True, False
            )
            ...
            diff.rio.to_raster(
                tmp_gtiff, compress="DEFLATE", tiled=True, dtype="float32"
            )
        ...
        # Compute categorical difference for DSWx products
            ...
            full_colormap = {
                ...
                10: (
                    ...
                    0,
                    255,
                ),  # 2 -> 2: Partial Surface Water -> Partial Surface Water (Black)
                15: (
                    0,
                    0,
                    0,
                    255,
                ),  # 3 -> 3: Inundated Vegetation -> Inundated Vegetation (Black)
                ...
                2: (
                    ...
                    80,
                    255,
                ),  # 2->0: Partial Surface Water -> Not Water (Lightest Red/Coral)
                ...
                9: (
                    ...
                    100,
                    255,
                ),  # 1->2: Open Water -> Partial Surface Water (Light Red)
                ...
                8: (
                    ...
                    237,
                    255,
                ),  # 0->2: Not Water -> Partial Surface Water (Lightest Blue)
                ...
                6: (
                    ...
                    255,
                    255,
                ),  # 2->1: Partial Surface Water -> Open Water (Light Blue)
            ...
            final_data_float = xr.where(keep_pixel_mask, transition_code, nd_float)
            ...
            # Write Temp GeoTIFF
            tmp_gtiff = out_path.with_suffix(".tmp.tif")
            final_data.rio.to_raster(
                tmp_gtiff, compress="DEFLATE", tiled=True, dtype="uint8"
            )
...
def compute_and_write_difference_positive_change_only(
    ...
    """
    logger.info(
        f"Computing generalized positive change (gain) layer for {out_path.name}..."
    )
    ...
    try:
        ...
        cond_intensification = (np.isin(da_early, partial_or_veg)) & (
            da_later == open_water
        )
        ...
        da_out = xr.DataArray(
            out_data, coords=da_later.coords, dims=da_later.dims, attrs=da_later.attrs
        )
        ...
        custom_colormap = {0: (255, 255, 255, 255), 1: (0, 0, 200, 255)}

        # Define class names for metadata
        class_names = {0: "No Change or Water Loss", 1: "Water Gain"}
...
def compute_and_write_max_flood_extent(
    ...
    da_out = xr.DataArray(out_data, coords={"y": base_y, "x": base_x}, dims=["y", "x"])
    ...
    class_names = {0: "Never Flooded", 1: "Maximum Flood Extent"}
...
def create_rtc_rgb_visualization(
    vv_path: Path | str, vh_path: Path | str, out_path: Path | str
) -> None:
    """
